...
So far the designed solution is supported to be installed in a machine running a late version of Ubuntu. It is designed so only a couple of manual steps are required, which are very simple and well explained in the \code{readme.md} of the project repository.

The installation process, however, assumes a specific set of requirements are met. These requirements are also listed in the \code{readme.md} and include having Apache 2, PHP (>= 5.6.4), and Composer to begin with. From here, the environment has also to fullfill Laravel's own requirements, which include:
...
Although it is not a mandatory requirement for Laravel, the \code{php5-sqlite} library is needed, as the default application uses SQLite for the database.
